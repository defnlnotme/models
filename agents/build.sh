...
echo ""
echo "┌─ Host notifications (KDE/Desktop) ─────────────────────────┐"
echo "│ To let container processes send desktop notifications to   │"
echo "│ the host KDE session, add these flags to 'docker run':     │"
echo "│                                                             │"
echo "│   -e DISPLAY=\"\${DISPLAY:-}\" \\                              │"
echo "│   -e DBUS_SESSION_BUS_ADDRESS=\"\${DBUS_SESSION_BUS_ADDRESS:-}\" \\│"
echo "│   -e XDG_RUNTIME_DIR=\"/run/user/\$(id -u)\" \\              │"
echo "│   -v /tmp/.X11-unix:/tmp/.X11-unix:rw \\                     │"
echo "│   -v /run/user/\$(id -u)/bus:/run/user/\$(id -u)/bus:ro     │"
echo "│   -v \$HOME/.Xauthority:/home/agent/.Xauthority:ro          │"
echo "│                                                             │"
echo "│ Inside the container you can then run:                      │"
echo "│   notify-send 'Hello from Docker' 'Container message'       │"
echo "│   kdialog --passivepopup 'Done!' 5                          │"
echo "└─────────────────────────────────────────────────────────────┘"
echo ""
